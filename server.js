...
// ---------------------------
// Carrega schema e docs do GitHub
// ---------------------------
...
async function loadRuleDocs(urls) {
  ...
  ruleDocsJoined = parts.join("\n\n---\n\n");
}
await loadRuleDocs(schema["x-rules"] || []);
...
app.get("/health", (_req, res) =>
  res.json({
    ok: true,
    schemaLoaded: !!schema,
    docsHash: simpleHash(ruleDocsJoined),
    hasWhitelistSource: !!(schema["x-wilmington"]?.whitelistSource),
    model: OPENAI_MODEL,
  })
);
app.get("/_docs", (_req, res) => res.type("text/plain").send(ruleDocsJoined));
...
app.post("/mutate", async (req, res) => {
  try {
    ...
    const foraDaWhitelistFiltrado = geo.foraDaWhitelist.filter((t) => !ignoreList.includes(t));

    const geoFail = (enforceWhitelist && foraDaWhitelistFiltrado.length > 0) || geo.termosBanidos.length > 0;
    ...
    const systemMsg = [
      ...
      "Responda EXATAMENTE no formato de tags solicitado. Não inclua as tags nos títulos ou shorts.",
...
`.trim();
    ...
    const relatorioSENTRY = {
      ...
      geografia: {
        whitelistAtivada: !!enforceWhitelist,
        locaisDetectados: geo.locaisDetectados,
        foraDaWhitelist: foraDaWhitelistFiltrado,
        termosBanidos: geo.termosBanidos,
      },
      dna: {
        personagensUsados: [],
      ...
      atmosfera: { elementosAplicados: [], faltantes: [] },
